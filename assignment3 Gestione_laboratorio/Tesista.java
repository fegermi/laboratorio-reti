...
public class Tesista implements Runnable, Utente  {
	...
	public void run() {      
		...
		lock.lock();
		System.out.println("[Tesista] " + this.nome + " ha occupato il " + this.assegnato[0].getNome() + " per " + this.tempo + " ms");
		try{
		...
		lock.unlock();
		System.out.println("[Tesista] " + this.nome + " ha liberato il " + this.assegnato[0].getNome());
	}
